  ...
  try {
    ...
    switch (action) {
      case 'createEvent': {
        ...
        const data = await base44.asServiceRole.entities.AdminEvent.create({
          name,
          type,
          isActive: true,
          config,
      ...
      case 'toggleEvent': {
        ...
        const data = await base44.asServiceRole.entities.AdminEvent.update(eventId, patch);
        return json({ success: true, event: data });
      }
      case 'createPet': {
        const required = ['name'];
        for (const k of required) if (!payload?.[k]) return json({ error: `${k} is required` }, 400);
        const data = await base44.asServiceRole.entities.CustomPet.create(payload);
        return json({ success: true, pet: data });
      }
      case 'createTheme': {
        const required = ['name', 'primaryColor', 'secondaryColor', 'accentColor', 'bgColor'];
        for (const k of required) if (!payload?.[k]) return json({ error: `${k} is required` }, 400);
        const data = await base44.asServiceRole.entities.CustomTheme.create(payload);
      ...
      case 'createShopItem': {
        ...
        const data = await base44.asServiceRole.entities.ShopItem.create(payload);
      ...
      case 'createBundle': {
        ...
        const data = await base44.asServiceRole.entities.Bundle.create(payload);
      ...
      case 'gift': {
        ...
        if (giftType === 'coins') {
          ...
          const updated = await base44.asServiceRole.entities.UserProfile.update(userRow.id, { questCoins });
        ...
        if (giftType === 'pet') {
          ...
          const updated = await base44.asServiceRole.entities.UserProfile.update(userRow.id, { unlockedPets });
          return json({ success: true, recipient: updated });
        }
        if (giftType === 'theme') {
          const themeId = value;
          ...
          const updated = await base44.asServiceRole.entities.UserProfile.update(userRow.id, { unlockedThemes });
          return json({ success: true, recipient: updated });
        }
        if (giftType === 'cosmetic') {
          ...
          const updated = await base44.asServiceRole.entities.UserProfile.update(userRow.id, { unlockedCosmetics });
          return json({ success: true, recipient: updated });
        }
        if (giftType === 'magic_egg') {
          if (!userRow.userId) return json({ error: 'recipient userId missing' }, 400);
          const egg = await base44.asServiceRole.entities.MagicEgg.create({ userId: userRow.userId });
